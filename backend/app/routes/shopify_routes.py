...
ENDPOINTS:
1. GET  /api/shopify/connect?shop=tienda → Inicia OAuth
2. GET  /api/shopify/callback            → Callback OAuth
3. GET  /api/shopify/status              → Estado de conexión
4. GET  /api/shopify/products            → Obtener productos
5. POST /api/shopify/import              → Importar a VendeFlow
6. POST /api/shopify/sync                → Sincronizar inventario
7. POST /api/shopify/create-product      → Crear producto desde IA ← NUEVO
8. DELETE /api/shopify/disconnect        → Desconectar tienda
"""
...
def shopify_callback():
    ...
    try:
        existing = PlatformConnection.query.filter_by(user_id=int(user_id), platform='shopify', store_name=shop).first()
        if existing:
...
def check_status():
    user_id = get_jwt_identity()
    connection = PlatformConnection.query.filter_by(user_id=int(user_id), platform='shopify', is_active=True).first()
    if not connection:
...
def import_from_shopify():
    ...
    log_sync(user_id=int(user_id), platform='shopify', action='import', status=status,
...
def sync_to_shopify():
    ...
    query = Product.query.filter(Product.user_id == int(user_id), Product.is_active == True, Product.shopify_id != None)
    if single_sku:
    ...
    for product in shopify_products:
        for variant in product.get('variants', []):
            variant_to_inventory[str(variant.get('id'))] = variant.get('inventory_item_id')
    synced = 0
    failed = 0
    errors = []
    for product in products:
        try:
            ...
            success, message = shopify_service.update_inventory(
                ...
                inventory_item_id=inventory_item_id, location_id=location_id, quantity=product.quantity
            )
            if success:
                synced += 1
            else:
    ...
    log_sync(user_id=int(user_id), platform='shopify', action='sync', status=status,
             sku=single_sku.upper() if single_sku else None, items_ok=synced, items_failed=failed,
             error_detail='; '.join(errors[:5]) if errors else None)
    return jsonify({
...
# ═══════════════════════════════════════════════════════════
# ENDPOINT: CREAR PRODUCTO DESDE IA ← NUEVO
# ═══════════════════════════════════════════════════════════
...
def create_product():
    """
    Crea un producto en Shopify con el contenido generado por la IA.

    ¿POR QUÉ SIEMPRE COMO BORRADOR?
    ---------------------------------
    El producto se crea como "draft" para que el usuario lo revise
    en el admin de Shopify antes de publicarlo. Esto evita publicar
    accidentalmente un producto incompleto o con errores.

    BODY:
    {
        "title":           "Título del producto",
        "body_html":       "<h2>Descripción HTML</h2>",
        "vendor":          "Marca",
    ...
    product, error = shopify_service.create_product(
        shop_name=connection.store_name,
        access_token=connection.access_token,
        product_data=data
    )

    if error:
        log_sync(int(user_id), 'shopify', 'create_product', 'error', error_detail=error)
        return jsonify({'success': False, 'error': error}), 400

    # Guardar también en VendeFlow si no existe
    try:
        existing = Product.query.filter_by(user_id=int(user_id), sku=data['sku'].upper(), is_active=True).first()
        if not existing:
        ...
        print(f"[Shopify] Error al guardar en VendeFlow: {e}")

    log_sync(int(user_id), 'shopify', 'create_product', 'success', items_ok=1)

    admin_url = f"https://{connection.store_name}.myshopify.com/admin/products/{product.get('id')}"

    return jsonify({
        'success': True,
        'message': '¡Producto creado como borrador en Shopify! Revísalo en tu admin antes de publicar.',
